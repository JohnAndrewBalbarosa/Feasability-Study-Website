...
export default function AnalyticsPage() {
  ...
  const resultHeaderLabel = useMemo(() => {
    if (analyticsRows.length === 0) {
      return "Needed Contribution To Break-even";
    }

    const allNegativeContribution = analyticsRows.every((row) => row.neededContributionToBreakEven < 0);
    const allPositiveOrZeroContribution = analyticsRows.every((row) => row.neededContributionToBreakEven >= 0);
              ...
              <tbody>
                {analyticsRows.map((row) => (
                  <tr key={row.key} className={row.status === "profit" ? "analytics-row-profit" : "analytics-row-loss"}>
                    ...
                    <td>
                      {row.neededContributionToBreakEven < 0
                        ? formatPhp(row.neededContributionToBreakEven)
                        : formatPhp(Math.max(-row.profitToday, 0))}
                    </td>
